...
def extract_text_with_page_numbers(pdf)->Tuple[str, list[Tuple[str, int]]]:
    ...
    return text, char_page_mapping # 每一个字对应的页码

# 处理文本创建向量存储
def process_text_with_splitters(text:str,char_page_mapping:list[int],save_path:str=None):
    """
    处理文本并创建向量存储
    :param text: 提取的文本内容
    :param char_page_mapping: 列表：每个字符对应的页码
    :param save_path: 可选，保存向量的路径
    ...
    # print(f'文本被分割成了{len(chunks)}块')

    # 创建嵌入模型 embedding 用于后面向量化
...
def load_knowledge_base(load_path:str,embeddings=None)->FAISS:
    ...
    :param embeddings: 可选，嵌入模型，如果为None，将创建一个新的DashScopeEmbeddings实例
...
# 读取文档
pdf_reader=PdfReader('./公司假期制度.pdf') # 这个文件流对象
text,char_page_mapping=extract_text_with_page_numbers(pdf_reader)
print(f'提取的文本长度：{len(text)}个字符')

# 处理文本并创建知识库，同时保存到磁盘
save_dir='./vector_db'
knowledgeBase=process_text_with_splitters(text,char_page_mapping,save_path=save_dir)
