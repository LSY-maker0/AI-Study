...
"""
    ...
    # 先切片，然后给每个切片生成问题，将问题拼接原来内容再塞进去，然后通过BM25建索引（原文和生成的问题）（，索引是放在BM25Okapi 这个数学模型对象里了
    def build_knowledge_index(self, knowledge_base):
        """构建知识库的BM25索引（包括原文和问题）"""
        print("正在构建知识库索引...")
        
        self.knowledge_base = knowledge_base
        content_documents = [] # 装原文
        question_documents = [] # 装问题
        content_metadata = [] # 装原文元数据 对其的扩充放这里
        question_metadata = [] # 装问题元数据 对其的扩充放这里
